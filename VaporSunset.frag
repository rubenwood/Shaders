...
vec4 getSunColor( in vec3 dir, inout float inside ) {
    ...
    float sunHeight = smoothstep(0.01 , 0.29 + iMouse.y, dir.z);
    ...
    sunWave = (1.0-smoothstep(sunHeight2+ (iMouse.x/100.0), 1.0, sunWave)) * (1.0 - sunHeight2) + sunHeight2;
    float sun = inside * sunWave +(iMouse.x / 100.0);
...
vec3 getWaveColor( in vec3 p, in vec3 projClosest, in vec3 projSecondClosest,
    ...
    float distanceToEdge = abs((projClosest.z-projSecondClosest.z)*3.0);
       ...
       // project the starting position to z = 0 so we ccan lower the raymarch count
        ...
        for (int steps = 0 ; steps < STEPS ; steps++) {
            ...
            if ( dist < 0.01 || steps == STEPS-1 ) {
                color = getWaveColor(p, projClosest, projSecondClosest, dir, totdist, vec2(fragCoord.x, fragCoord.y));
                break;
            }
        }
    }
