...
uniform vec3 mirror; // mirror manipulator
uniform float glow; // global glow
uniform float luminosity; // luminosity of aurora
uniform float waterOpacity;
uniform vec3 waterCol; //(0.3,0.3,0.5)
...
void main(){
    ...
    vec2 p = q - 0.5;
	p.x *= resolution.x*uvScale.x/resolution.y*uvScale.y;
    ...
    else //Reflections
    {
        ...
        col += mix(vec3(0.2,0.25,0.5)*0.08,waterCol*waterOpacity, nz2*0.4);
    }
    
    vec4 fc = vec4(col*glow, 1.);
